package com.windcoder.qycms.system.utils;
...
public class IpWhilteUtil {

    // ip白名单本地缓存
    private static Set<String> ipConfigSet = new HashSet<String>();
     ...
     * @param listStr
     */
    public static void refresh(String listStr) {
        if(listStr == null) {
            ipConfigSet = new HashSet<String>(1);
        } else {
            String listArray[] = listStr.split("\\r\\n|\\n|\\r");
            ipConfigSet = getAvaliIpList(listArray);
        }
    }
    ...
    private static Set<String> getAvaliIpList( String[] allowIp) {
        String[] splitRex = allowIp;// 拆分出白名单正则
     ...
     * @return
     */
    public static boolean isPermited(String ip) {
        return isPermited(ip, ipConfigSet);
    }
    /**
     * isPermited:(根据IP,及可用Ip列表来判断ip是否包含在白名单之中).
     * @param ip
