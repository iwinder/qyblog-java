...
-- hW61c
INSERT INTO `sys_user` (`id`, `created_date`, `last_modified_date`, `avatar`, `email`, `deleted`, `disable`, `nickname`, `password`, `salt`, `username`, `created_by`, `last_modified_by`) VALUES (1, now(), now(), '/content/upload/2018/11/04/e246ba56-81aa-4226-8189-cc2d10ccd56a.jpg', '1@qq.com', 0, 0, '测试', '0e8b34967694c12c893f4906023f69f43a701e6f', '52084c3100d2b5c6bdcc3b79c8d9dee69a5450454d30', 'admin', 1, 1);
...
CREATE TABLE `blog_article` (
  ...
  `status` varchar(255) DEFAULT 'draft' comment '状态:public: 公共，private: 私密，encryption：加密',
...
CREATE TABLE `sns_comment` (
  ...
  `type` varchar(255) DEFAULT NULL comment '类型 GENERAL:正常，pingBack：通知',
  `target_id` bigint(20) DEFAULT NULL comment '评论代理',
  `depth` int(11) DEFAULT NULL comment '评论层级',
  `parent_id` bigint(20) DEFAULT 0 comment '父评论',
  `user_id` bigint(20) DEFAULT 0 comment '用户id, 0 游客',
  `top_parent_id` bigint(20) DEFAULT 0 comment '根回复',
  `created_date` datetime DEFAULT now() comment '创建时间',
  `last_modified_date` datetime DEFAULT now() comment '更新时间',
  PRIMARY KEY (`id`),
...
CREATE TABLE `sys_menus_agent` (
  ...
  `identifier` int(11) DEFAULT 0  comment '菜单位置：0未启用， 1顶部，2页尾',
   `type` varchar(255)  DEFAULT NULL comment '类型',
  `created_by` bigint(20) DEFAULT null comment '创建者',
  `last_modified_by` bigint(20) DEFAULT NULL comment '更新者',
  `created_date` datetime DEFAULT now() comment '创建时间',
  `last_modified_date` datetime DEFAULT now() comment '更新时间',
  PRIMARY KEY (`id`),
  KEY sys_menus_agent_identifier_IDX (`identifier`)  USING BTREE
) ENGINE=InnoDB  DEFAULT CHARSET=utf8mb4 COMMENT='菜单包装表';

select min(id) into @v_system_user_id from sys_user;
Insert into sys_menus_agent (name,identifier,type,created_by,last_modified_by)
values('顶部菜单',1,'SYSTEM',@v_system_user_id,@v_system_user_id) ;
Insert into sys_menus_agent (name,identifier,type,created_by,last_modified_by)
values('页脚菜单',2,'SYSTEM',@v_system_user_id,@v_system_user_id) ;
...
-- 媒体库类型表
drop table if exists `file_lib_type`;
CREATE TABLE `file_lib_type` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT comment 'id',
  `name` varchar(255) DEFAULT NULL comment '媒体库类型名称',
	`identifier` int(11) DEFAULT 0  comment '类型标识：1本地， 2七牛，3阿里OSS',
	`type` varchar(255)  DEFAULT NULL comment '类型',
  `status` bit(1) DEFAULT b'0' comment '是否启用：0不启用， 1 启用',
  `created_by` bigint(20) DEFAULT null comment '创建者',
  `last_modified_by` bigint(20) DEFAULT NULL comment '更新者',
  `created_date` datetime DEFAULT now() comment '创建时间',
  `last_modified_date` datetime DEFAULT now() comment '更新时间',
...
CREATE TABLE `file_lib_config` (
  ...
  `type_id` bigint(20) NOT NULL DEFAULT 0 comment '媒体库类型ID',
  `created_by` bigint(20) DEFAULT null comment '创建者',
  `last_modified_by` bigint(20) DEFAULT NULL comment '更新者',
  `created_date` datetime DEFAULT now() comment '创建时间',
  `last_modified_date` datetime DEFAULT now() comment '更新时间',
  PRIMARY KEY (`id`),
  UNIQUE INDEX file_lib_config_type_id_idx(type_id ASC)
) ENGINE=InnoDB  DEFAULT CHARSET=utf8mb4 COMMENT='媒体库类型配置';
...
CREATE TABLE `file_meta` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT comment 'id',
  `origin_file_name` varchar(255) DEFAULT NULL comment '原始名称',
  `fname` varchar(255) DEFAULT NULL  comment '文件名',
  `fsize` bigint(20)  DEFAULT 0 comment '文件大小',
  `extention` varchar(255) DEFAULT NULL comment '文件扩展名',
  `mime_type` varchar(255) DEFAULT NULL comment '资源的 MIME 类型',
  `fhash` varchar(255) DEFAULT NULL comment '文件的HASH值',
  `fmd5` varchar(255) DEFAULT NULL comment '文件md5值',
  `relative_path` varchar(255) DEFAULT NULL comment '通过浏览器访问的相对路径',
  `created_by` bigint(20) DEFAULT null comment '创建者',
  `last_modified_by` bigint(20) DEFAULT NULL comment '更新者',
  `created_date` datetime DEFAULT now() comment '创建时间',
  `last_modified_date` datetime DEFAULT now() comment '更新时间',
