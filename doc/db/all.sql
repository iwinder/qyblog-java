...
CREATE TABLE `file_meta` (
  ...
  `extention` bigint(20)  DEFAULT 0 comment '文件扩展名',
  `mime_type` bit(1) DEFAULT b'0' comment '资源的 MIME 类型',
  `fhash` bit(1) DEFAULT b'0' comment '文件的HASH值',
  `fmd5` bit(1) DEFAULT b'0' comment '文件md5值',
  `relative_path` bigint(20) DEFAULT NULL comment '通过浏览器访问的相对路径',
  `created_by` bigint(20) DEFAULT null comment '创建者',
  `created_date` datetime DEFAULT now() comment '创建时间',
  `last_modified_date` datetime DEFAULT now() comment '更新时间',
  PRIMARY KEY (`id`)
